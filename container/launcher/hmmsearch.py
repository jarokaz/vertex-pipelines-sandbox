...
RUNNER_SCRIPT = '/scripts/alphafold_runners/hmmsearch_runner.py'
...
def run_hmmsearch(
    ...
    logging.info('Starting Hmmsearch runner')
    ...
    env_variables = {
        ...
        'DB_ROOT': mount_path,
        'TEMPLATE_DB_PATH': ref_databases['metadata'][params['template_db']],
        'MAX_TEMPLATE_HITS': params['max_template_hits'],
        'MAX_TEMPLATE_DATE': params['max_template_date'],
    }
    display_name = f'HMMSEARCH_JOB_{time.strftime("%Y%m%d_%H%M%S")}'
    ...
    with  open(params["template_hits_path"], 'r') as f:
        template_hits_sto = f.read()
    parsed_sto = parsers.parse_stockholm(template_hits_sto)
    metadata = {}
    metadata['Number of hits'] = len(parsed_sto)
